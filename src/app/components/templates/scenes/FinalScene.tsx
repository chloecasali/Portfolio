...
export function FinalScene({ content, onBackToTop, ui }: FinalSceneProps) {
  return (
      ...
      <div className="relative z-10 max-w-4xl px-6 text-center">
        <motion.div initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: true }}>
          ...
          <motion.div className="mb-16 flex items-center justify-center gap-6" initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }} viewport={{ once: true }}>
            <motion.a href="mailto:contact@example.com" className="flex h-14 w-14 items-center justify-center rounded-full border border-black/10 bg-black/5 backdrop-blur-sm transition-all duration-300 hover:border-[#FA00C4] hover:bg-[#FA00C4] dark:border-white/20 dark:bg-white/10" whileHover={{ scale: 1.1, y: -5 }} whileTap={{ scale: 0.95 }}><Mail className="h-5 w-5 text-black dark:text-white" /></motion.a>
            <motion.a href="https://linkedin.com" target="_blank" rel="noopener noreferrer" className="flex h-14 w-14 items-center justify-center rounded-full border border-black/10 bg-black/5 backdrop-blur-sm transition-all duration-300 hover:border-[#FA00C4] hover:bg-[#FA00C4] dark:border-white/20 dark:bg-white/10" whileHover={{ scale: 1.1, y: -5 }} whileTap={{ scale: 0.95 }}><Linkedin className="h-5 w-5 text-black dark:text-white" /></motion.a>
            <motion.a href="https://github.com" target="_blank" rel="noopener noreferrer" className="flex h-14 w-14 items-center justify-center rounded-full border border-black/10 bg-black/5 backdrop-blur-sm transition-all duration-300 hover:border-[#FA00C4] hover:bg-[#FA00C4] dark:border-white/20 dark:bg-white/10" whileHover={{ scale: 1.1, y: -5 }} whileTap={{ scale: 0.95 }}><Github className="h-5 w-5 text-black dark:text-white" /></motion.a>
          </motion.div>
